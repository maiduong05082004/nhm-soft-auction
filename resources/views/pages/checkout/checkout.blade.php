                            ...
                            <div class="grid grid-cols-1 gap-4">
                                ...
                                <div>
                                    ...
                                    <textarea name="note" class="textarea textarea-bordered w-full" rows="3"
                                        placeholder="Ghi chú về thời gian giao hàng, địa điểm, ...">{{ old('note', $auctionCheckoutData['note'] ?? '') }}</textarea>
                                </div>
                            </div>
                        </div>
                    </div>
                                ...
                                <label class="flex items-center gap-3 p-3 rounded-lg border cursor-pointer">
                                    ...
                                    <div>
                                        ...
                                        @if (isset($hasCreditCard) && !$hasCreditCard)
                                            <div class="text-xs text-red-500 mt-1">Chưa cấu hình chuyển khoản ngân hàng, vui
                                                lòng chọn COD.</div>
                                        @endif
                                    </div>
                                </label>
                    ...
                    <div class="card-body">
                                    ...
                                    <div class="avatar">
                                        <div class="w-12 h-12 rounded-md overflow-hidden">
                                            ...
                                            <img src="{{ $img ? \App\Utils\HelperFunc::generateURLFilePath($img) : asset('images/product_default.jpg') }}"
                                                alt="{{ $product->product->name ?? 'Sản phẩm trả giá' }}"
                                                class="w-full h-full object-cover">
                                        </div>
                                    </div>
                                    <div class="flex-1">
                                        <div class="text-sm font-medium line-clamp-1">
                                            {{ $cartItems->first()->product->name ?? 'Sản phẩm trả giá' }}</div>
                                ...
                                @foreach ($cartItems as $item)
                                    <div class="py-3 flex items-center gap-3">
                                        ...
                                        <div class="text-sm font-semibold flex flex-col">
                                            ...
                                            ₫
                                            @if (
                                                $item->product?->owner->creditCard ||
                                                    ($item->product?->owner->hasRole(App\Enums\Permission\RoleConstant::ADMIN) &&
                                                        $item->product?->pay_method != \App\Enums\Product\ProductPaymentMethod::COD->value))
                                                <span class="text-green-600 text-xs">Có thể thanh toán online!</span>
                                            @else
                                                <span class="text-red-600 text-xs">Không thể thanh toán online!</span>
                                            @endif
                                        </div>
                                    </div>
                                @endforeach
                        ...
                        <div class="mt-4 space-y-2 text-sm">
                            ...
                            @if (isset($discountInfo) && $discountInfo['has_discount'])
                                <div class="flex justify-between">
                                    <span class="text-[#6c6a69] font-bold">Giảm giá
